  ...
  if (!muterole) {
    ...
    .addField('Reason', reason);
     message.channel.send({
      embed: {
        color: 15158332,
        description: `:white_check_mark: Success! I\'ve Muted that user.`
      }})
...
stream.write(`${dformat}  ` + `[ChitterEngine]: ${message.author.username}#${message.author.discriminator} Muted User ${user.id}` + "\n");
stream.end(); 
};
